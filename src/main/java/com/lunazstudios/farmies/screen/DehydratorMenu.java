...
public class DehydratorMenu extends AbstractContainerMenu {
    ...
    public DehydratorMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public DehydratorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(FMenus.DEHYDRATOR_MENU.get(), pContainerId);
        blockEntity = ((DehydratorBlockEntity) entity);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, FBlocks.DEHYDRATOR.get());
    }
