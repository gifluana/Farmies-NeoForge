...
public record FryingPanRecipe(
        NonNullList<Ingredient> ingredients,
        ItemStack outputItem
) implements Recipe<FryingPanRecipeInput> {
    ...
    @Override
    public boolean matches(@NotNull FryingPanRecipeInput input, Level level) {
    ...
    @Override
    public ItemStack assemble(FryingPanRecipeInput input, HolderLookup.Provider provider) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return FRecipes.FRYING_PAN_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return FRecipes.FRYING_PAN_TYPE.get();
    }

    public static class Serializer implements RecipeSerializer<FryingPanRecipe> {

        public static final MapCodec<FryingPanRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
                Ingredient.CODEC_NONEMPTY.listOf().fieldOf("ingredients").xmap(
                        ...
                        ArrayList::new
                ).forGetter(FryingPanRecipe::ingredients),
                ItemStack.CODEC.fieldOf("result").forGetter(FryingPanRecipe::outputItem)
        ).apply(inst, FryingPanRecipe::new));
        ...
        public static final StreamCodec<RegistryFriendlyByteBuf, FryingPanRecipe> STREAM_CODEC =
                StreamCodec.of(
                        ...
                        buf -> {
                            ...
                            return new FryingPanRecipe(ingredients, output);
                        }
                );
